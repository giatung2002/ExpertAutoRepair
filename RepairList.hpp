...
_nowServicing = std::next(_nowServicing);
...
{
std::list<Repair>::iterator itr = _nowServicing;

itr++;
_dailyList.insert(std::next( _nowServicing), newRequest);
...
//_nowServicing = std::previous(_nowServicing);
...
{
int position = 1;
for (std::list<Repair>::iterator itr = _dailyList.begin(); itr != _dailyList.end(); itr++){
	if(position==number)
	return itr->vehicle();
	position++;
}
...
{
double maxCost = _dailyList.begin()->cost();
for(std::list<Repair>::iterator it = _dailyList.begin()++; it != _dailyList.end(); it++)
if (it->cost() > maxCost)
maxCost = it-> cost();
return maxCost;
}
